...
from test_agents.TRPOAgent.utils.torch_utils import use_cuda, Tensor, Variable, ValueFunctionWrapper
import test_agents.TRPOAgent.utils.math_utils as math_utils
from test_agents.TRPOAgent.models import DQNSoftmax
...
class Agent:
    ...
    opponent: Agent
      agent to use as AI opponent for TRPO
    ...
    """
        ...
        self.env = None
        self.opponent = None
        self.policy_model = DQNSoftmax(3)
        self.value_function_model = None
    ...
    def load_model(self):
        ...
        self.policy_model.load_state_dict(weights, strict=False)
    ...
    """
        observation_tensor = torch.tensor(np.float64(observation))
    ...
    """
        ...
        num_batches = int(len(all_actions) / self.batch_size if len(
        ...
        for batch_num in range(num_batches):
            ...
            self.observations = torch.cat(
                [self.preprocess(Variable(Tensor(observation)).unsqueeze(0)) for observation in self.observations])
            ...
            if policy_gradient.nonzero().size()[0]:
                ...
                gdotstepdir = -policy_gradient.dot(step_direction_variable).data.numpy()
                ...
                diagnostics = collections.OrderedDict(
                    [('Total Reward', total_reward), ('KL Old New', kl_old_new.data.numpy()), (
                        'Entropy', self.entropy.data.numpy()), ('EV Before', ev_before), ('EV After', ev_after)])
